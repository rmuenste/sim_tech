...
\begin{frame}
  \frametitle{Sources}
    ...
    \begin{itemize}
      \item Insert geometric shapes into the pipeline (boxes, spheres, cylinders, ...)
      \item Can be transformed by \keyword{Transform} filter
      \item Also text, programmable sources or measurement tools
    \end{itemize}
    \begin{center}
      \includegraphics[width=0.8\textwidth]{screenshots/sources.png}
    \end{center}
...
%      \item PvPython simple example location: \kommandozeile{/mypath/to/examples/pv\_examples/PvPython/paraview\_python}
      ...
      \item Prefer \keyword{pvbatch} over \keyword{pvpython} as pvpython tries to open an X windows which may fail on some computers    
  \end{itemize}
